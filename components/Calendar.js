...
export default function Calendar({demo, completeData, handleSetMood}) {
    ...
    function handleIncrementMonth(val) {
        ...
        if (numericMonth + val < 0) {
            ...
            // set month = 0 and increment year
            setSelectedYear(curr => curr + 1);
  ...
  return (
    <div className='flex flex-col gap-2'>
        <div className='grid grid-cols-5 gap-4'>
            <button onClick={() => handleIncrementMonth(-1)} className="mr-auto text-indigo-400 text-lg sm:text-xl duration-200 hover:opacity-60"><i className="fa-solid fa-circle-chevron-left"></i></button>
            ...
            <button onClick={() => handleIncrementMonth(1)} className='ml-auto text-indigo-400 text-lg sm:text-xl duration-200 hover:opacity-60'><i className="fa-solid fa-circle-chevron-right"></i></button>
        </div>
